...
# Aislamiento de Datos por RFC (Evita cruce de números)
                                ...
                                if not cer_file or not key_file or not pass_sat:
                                    ...
                                    with st.spinner("Conectando con el SAT para revisión y descarga invisible..."):
                                        try:
                                            ...
                                            if estado == '3': # Terminada y lista para descargar
                                                paquetes = res_verifica.get('ids_paquetes', [])
                                                if not paquetes:
                                                    ...
                                                    if facturas_extraidas:
                                                        # Evitar duplicados consultando qué UUIDs ya tenemos
                    ...
                    with st.spinner("🔄 Conectando directo al SAT con tu e.firma..."):
                        try:
                            ...
                            if id_solicitud:
                                ...
                                st.error(f"❌ Rechazado por el SAT (Verifica que sea la FIEL de {RFC_ACTIVO}). Respuesta SAT: {respuesta}")
                        except Exception as ex:

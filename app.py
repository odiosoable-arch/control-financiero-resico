...
# ==============================================================================
    ...
    with tabs_carlos[0]:
        ...
        with c1: st.markdown(f"""<div class="metric-card" style="border-left: 4px solid #e74a3b;"><div class="metric-title">IVA Neto a Pagar</div><div class="metric-value" style="color: #e74a3b;">${iva_por_pagar_c:,.2f}</div></div>""", unsafe_allow_html=True)
        with c2: st.markdown(f"""<div class="metric-card" style="border-left: 4px solid #f6c23e;"><div class="metric-title">IVA Facturado (Ingresos)</div><div class="metric-value" style="color: #f4b619;">${t_iva_ingresos_c:,.2f}</div></div>""", unsafe_allow_html=True)
        with c3: st.markdown(f"""<div class="metric-card" style="border-left: 4px solid #36b9cc;"><div class="metric-title">IVA Acreditable (Gastos)</div><div class="metric-value" style="color: #36b9cc;">${t_iva_gastos_c:,.2f}</div></div>""", unsafe_allow_html=True)
        with c4: st.markdown(f"""<div class="metric-card" style="border-left: 4px solid #1cc88a;"><div class="metric-title">Total Ingresos</div><div class="metric-value" style="color: #1cc88a;">${t_ingresos_c:,.2f}</div></div>""", unsafe_allow_html=True)
        with c5: st.markdown(f"""<div class="metric-card"><div class="metric-title">Total Egresos</div><div class="metric-value">${t_gastos_c:,.2f}</div></div>""", unsafe_allow_html=True)
        
        st.write("")
        if iva_por_pagar_c > 0:
            monto_necesario_gastos = iva_por_pagar_c / 0.16
            st.info(f"💡 **Estrategia Fiscal:** Para que tu IVA Neto a Pagar baje a cero, necesitas facturar aproximadamente **${monto_necesario_gastos:,.2f}** adicionales en gastos (antes de IVA) durante este mes.")
        else:
            st.success("✅ **¡Felicidades!** Tu IVA a pagar está completamente cubierto y balanceado con tus gastos actuales.")

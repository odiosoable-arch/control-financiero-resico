...
# Función para procesar y leer los XML (CFDIs) en memoria
def procesar_cfdi(xml_content, rfc_activo):
    try:
        ...
        uuid = None
        for timbre in root.iter('TimbreFiscalDigital'):
        ...
        rfc_emisor = emisor.attrib.get('Rfc', '').upper() if emisor is not None else ''
        rfc_receptor = receptor.attrib.get('Rfc', '').upper() if receptor is not None else ''
        
        if rfc_emisor == rfc_activo: tipo = 'INGRESO'
        elif rfc_receptor == rfc_activo: tipo = 'GASTO'
        else: return None 
        ...
        if impuestos is not None:
            traslados = impuestos.find('Traslados')
            if traslados is not None:
                for traslado in traslados.iter('Traslado'):
                    impuesto = traslado.attrib.get('Impuesto')
                    if impuesto == '002': # 002 representa el IVA en el anexo del SAT
...
if df_global.empty:
    ...
    df_global = pd.DataFrame(columns=['id', 'uuid_cfdi', 'rfc_emisor', 'rfc_receptor', 'fecha_emision', 'tipo', 'subtotal', 'impuestos', 'total'])
else:
...
if 'rfc_emisor' in df_filtrado_fecha.columns and 'rfc_receptor' in df_filtrado_fecha.columns:
    df_entorno = df_filtrado_fecha[(df_filtrado_fecha['rfc_emisor'].str.upper() == RFC_ACTIVO) | (df_filtrado_fecha['rfc_receptor'].str.upper() == RFC_ACTIVO)]
else:
    if perfil_activo == "Operación Nicole":
        df_entorno = df_filtrado_fecha.copy()
    else:
        df_entorno = pd.DataFrame(columns=df_filtrado_fecha.columns)
...
if perfil_activo == "Operación Nicole":
    ...
    if puede_ver_sat:
        with get_tab("📥 Conexión SAT"):
            ...
            st.markdown(f"Módulo exclusivo para Administradores y Contadores. Valida el RFC de la cuenta activa: **`{RFC_ACTIVO}`**.")
                    ...
                    if not cer_file or not key_file or not pass_sat:
                        ...
                        with st.spinner("🔄 Conectando directamente con los Web Services del SAT..."):
                            try:
                                ...
                                # SOLUCIÓN: Limpieza estricta de microsegundos para evitar errores del SAT
                                ...
                                if id_solicitud_generado:
                                    ...
                                    st.error(f"❌ El SAT rechazó la petición. Respuesta SAT: {respuesta}")
                ...
                for idx, fila in df_solicitudes.iterrows():
                    ...
                    with st.expander(f"Solicitud: {tipo_req} ({fila['fecha_inicio']} a {fila['fecha_fin']}) | Estatus: {fila['estatus']}"):
                        ...
                        if fila['estatus'] == 'Pendiente':
                            if st.button(f"🔄 Verificar Estatus y Extraer Facturas", key=f"btn_check_{fila['id_solicitud_sat']}"):
                                if not cer_file or not key_file or not pass_sat:
                                    ...
                                    with st.spinner("Conectando con el SAT para revisión y descarga invisible..."):
                                        try:
                                            ...
                                            if estado == '3':
                                                # SOLUCIÓN: Lectura dinámica de llaves de paquetes en cfdiclient
                                                paquetes = []
                                                ...
                                                if not paquetes:
                                                    ...
                                                    if facturas_extraidas:
                                                        ...
                                                        if nuevas:
                                                            ...
                                                            st.success(f"✅ ¡Magia completada! Se guardaron {len(nuevas)} facturas nuevas a tu base de datos.")
                                                        else:
                                                            st.success("✅ Paquete procesado. Todas las facturas de este paquete ya estaban en tu base de datos.")
                            ...
                            st.success(f"Esta solicitud ya fue procesada y clasificada como: {fila['estatus']}.")
            else:
...
# ==============================================================================
    ...
    t_ingresos_c = ingresos_df_c['total'].sum() if not ingresos_df_c.empty else 0
    t_gastos_c = gastos_df_c['total'].sum() if not gastos_df_c.empty else 0
    t_iva_ingresos_c = ingresos_df_c['impuestos'].sum() if not ingresos_df_c.empty else 0
    t_iva_gastos_c = gastos_df_c['impuestos'].sum() if not gastos_df_c.empty else 0
                ...
                if not cer_file or not key_file or not pass_sat:
                    ...
                    with st.spinner("🔄 Conectando directo al SAT con tu e.firma..."):
                        try:
                            ...
                            # SOLUCIÓN: Limpieza de microsegundos
                            f_ini = datetime.datetime(fecha_inicio.year, fecha_inicio.month, fecha_inicio.day, 0, 0, 0)
                            f_fin = datetime.datetime(fecha_fin.year, fecha_fin.month, fecha_fin.day, 23, 59, 59)
        ...
        if not df_solicitudes.empty:
            for idx, fila in df_solicitudes.iterrows():
                ...
                with st.expander(f"Solicitud: {tipo_req} ({fila['fecha_inicio']} a {fila['fecha_fin']}) | Estatus: {fila['estatus']}"):
                    st.write(f"**ID SAT:** `{fila['id_solicitud_sat']}`")
                    if fila['estatus'] == 'Pendiente':
                        if st.button(f"🔄 Verificar Estatus y Extraer Facturas", key=f"btn_check_carlos_{fila['id_solicitud_sat']}"):
                            if not cer_file or not key_file or not pass_sat:
                                            ...
                                            if not paquetes:
                                                ...
                                                if facturas_extraidas:
                                                    ...
                                                    if nuevas:
                                                        ...
                                                        st.success("✅ Paquete procesado. Todas las facturas ya estaban en tu base de datos.")

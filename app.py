...
if df.empty:
    ...
    if is_admin:
        ...
        # --- PESTAÑA: CONEXIÓN SAT (SÓLO ADMIN) ---
        with get_tab("📥 Conexión SAT"):
            st.title("📥 Sincronización Directa con el SAT")
            st.markdown("Solicita y descarga masivamente tus XML directo de los servidores del SAT usando tu e.firma o CSD. **(Tus archivos no se guardan en el disco, todo el proceso de firma es seguro y ocurre en la memoria virtual).**")

            # Área de carga de credenciales
            st.markdown("### 🔐 Credenciales Fiscales")
            ...
            pass_sat = st.text_input("Contraseña de la Clave Privada", type="password", help="La contraseña de tu e.firma o Sellos (CSD)")

            st.divider()

            # --- PASO 1: SOLICITAR ---
            st.markdown("### 1️⃣ Paso 1: Solicitar Paquete de XML al SAT")
            st.info("El SAT recibe la petición y te asigna un ID. Puede tardar de 5 minutos a un par de horas en preparar tu paquete.")
            ...
            with st.form("form_solicitud_sat"):
                ...
                if btn_solicitar:
                    if not cer_file or not key_file or not pass_sat:
                        ...
                        with st.spinner("🔄 Autenticando y conectando con los Web Services del SAT..."):
                            try:
                                # 1. Leer bytes directos y limpiar espacios accidentales en la contraseña
                                cer_der = cer_file.getvalue()
                                key_der = key_file.getvalue()
                                pass_limpia = pass_sat.strip() 

                                # 2. Importar las clases correctas de cfdiclient
                                from cfdiclient import Fiel, Autenticacion, SolicitaDescargaEmitidos, SolicitaDescargaRecibidos

                                RFC_SOLICITANTE = "GAMN050420SZ1" # Reemplaza por tu RFC si es diferente
                                
                                # 3. Iniciar la firma electrónica y el token del SAT
                                fiel = Fiel(cer_der, key_der, pass_limpia)
                                token = Autenticacion(fiel).obtener_token()

                                # Formatear fechas al estándar estricto que exige Hacienda
                                ...
                                # Crear la solicitud dependiendo si es de Ingreso o Gasto
                                ...
                                # Obtener el número de ticket que nos regrese el SAT
                                id_solicitud_generado = respuesta.get('id_solicitud')

                                if id_solicitud_generado:
                                    # Guardar en base de datos el ID del ticket para luego reclamarlo
                                    ...
                                    st.error(f"❌ El SAT rechazó la petición. (Verifica si tus sellos están vigentes o las fechas). Respuesta SAT: {respuesta}")

                            except Exception as ex:
                                st.error(f"❌ Error interno al comunicarse con el SAT: {ex}")

            st.divider()

            # --- PASO 2: VERIFICAR Y DESCARGAR ---
            st.markdown("### 2️⃣ Paso 2: Verificar Solicitudes y Descargar")
            st.write("Consulta el estatus de los paquetes que le pediste al SAT. Si están listos, descárgalos directo a tu base de datos.")
            ...
            if not df_solicitudes.empty:
                for idx, fila in df_solicitudes.iterrows():
                    with st.expander(f"Solicitud: {fila['tipo_solicitud']} ({fila['fecha_inicio']} a {fila['fecha_fin']}) | Estatus: {fila['estatus']}"):
                        ...
                        if fila['estatus'] == 'Pendiente':
                            if st.button(f"🔄 Verificar Estatus en el SAT", key=f"btn_check_{fila['id_solicitud_sat']}"):
                                if not cer_file or not key_file or not pass_sat:
                                    st.error("⚠️ Sube tus credenciales arriba para poder preguntarle al SAT por este paquete.")
                                else:
                                    st.info("Verificando paquete en los servidores del SAT...")
                                    # Aquí procesaremos la descarga del ZIP cuando el SAT lo libere
                        else:
                            st.success("Esta solicitud ya fue descargada e ingresada a tu contabilidad.")
            else:
